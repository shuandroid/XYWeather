 ...
 * </p>
 */
public class MyPagerTitleStrip extends ViewGroup implements MyViewPager.Decor {
	...
	private class CircleIndicator extends View implements MyViewPager.OnPageChangeListener{
		...
		public CircleIndicator(Context context, AttributeSet attrs) {
			...
			if (attrs == null)
				return;
			TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.MyPagerTitleStrip);
			mIndicatorRadius = typedArray.getDimensionPixelSize(R.styleable.MyPagerTitleStrip_pts_radius,
					DEFAULT_INDICATOR_RADIUS);
			mIndicatorMargin = typedArray.getDimensionPixelSize(R.styleable.MyPagerTitleStrip_pts_margin,
					DEFAULT_INDICATOR_MARGIN);
			mIndicatorBackground = typedArray.getColor(R.styleable.MyPagerTitleStrip_pts_background,
					DEFAULT_INDICATOR_BACKGROUND);
			mIndicatorSelectedBackground = typedArray.getColor(R.styleable.MyPagerTitleStrip_pts_selected_background,
...
//			Log.e("CircleIndicator", "onPageScrollStateChanged()->" + state );
			switch (state) {
			case MyViewPager.SCROLL_STATE_IDLE:
				hideSelf();
				break;
			default:
				showSelf();
...
//			canvas.restoreToCount(sc);
	...
	protected void onAttachedToWindow() {
		...
		final MyViewPager pager = (MyViewPager) parent;
...
//		final int contentWidth = prevWidth + currWidth + nextWidth;// stripWidth
	...
	private class PageListener extends DataSetObserver implements MyViewPager.OnPageChangeListener,
			MyViewPager.OnAdapterChangeListener {
		...
		@Override
		public void onPageSelected(int position) {
			if (mScrollState == MyViewPager.SCROLL_STATE_IDLE) {
